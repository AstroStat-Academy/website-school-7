# Astrostat School Rome 2026 — Website
...
1. Go to https://ui.adsabs.harvard.edu and log in
...
function doPost(e) {
  try {
    ...
    if (p['entry.312559857']) {
      try {
        MailApp.sendEmail(
          p['entry.312559857'], // Email
          'Application received – AstroStat School Rome 2026',
          'Dear ' + (p['entry.905896922'] || 'Applicant') + ',\n\n' +
          'Thank you for applying to the AstroStat School Rome 2026. ' +
...
### Authorization (required once)

Because the script sends emails, it must be manually authorized:

1. In the Apps Script editor, add this temporary function:

```javascript
function auth() {
  MailApp.sendEmail('your@email.com', 'auth', 'auth');
}
```

2. In the top toolbar:
   - Select `auth` from the function dropdown
   - Click ▶ Run
3. Approve the permission dialog
...
4. Click Deploy
5. Copy the URL (format: https://script.google.com/macros/s/.../exec)

### Connecting the form

In `apply.html`, update the form `action` attribute and the `fetch` call in the script to point to the Apps Script URL obtained above.
