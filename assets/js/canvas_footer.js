/**
 * canvas_footer.js — Star-face footer canvas
 *
 * Self-contained IIFE. Renders the star-face animation on <canvas id="footer-c">.
 * Starts immediately in face+bite state (no morph animation).
 * Uses an IntersectionObserver on #apply-section to show/hide itself.
 *
 * Exposes window.FooterCanvas = { init() }
 */
(function () {
  'use strict';
  ...
  const BITE_OPEN_END = 0.55;
  const BITE_SHUT_END = 0.80;
  const BITE_MAX_JAW  = 0.058;
  const BITE_START_AT = 0.99;   // kept for symmetry; not used in the no-morph path

  // ── Orbit speed ───────────────────────────────────────────────────────────────
  const ORBIT_FACE_BASE   = 0.0005;
  const ORBIT_BG_BASE     = 0.0001;
  const ORBIT_INNER_BOOST = 0.0006;
  ...
  // ── Star creation — face stars start at morphT = 1 ───────────────────────────
  function createStars() {
    ...
    fp.forEach(p => {
      ...
      const dist  = 60 + Math.random() * (Math.min(W, H) * 0.46);
      ...
      stars.push({
        ...
        baseOpacity: isEye  ? 0.55 + Math.random() * 0.3
                   : isTooth ? 0.50 + Math.random() * 0.3
  ...
  function loop() {
    if (!visible) {
      // Keep time ticking so cascade timers stay coherent, but skip drawing
      time++;
    ...
    // Eye glow — always fully on (morphT=1 from the start)
    const ep = (0.68 + Math.sin(t * 1.7) * 0.12 + Math.sin(t * 3.3) * 0.05) * EYE_GLOW_INTENSITY;
    ...
    for (const s of stars) {
      ...
      if (s.assigned) {
        ...
        // morphT is always 1 — stars are fully at their face positions
        const jx = Math.sin(time * s.jFreq + s.jPhaseX) * s.jAmp;
        const jy = Math.cos(time * s.jFreq + s.jPhaseY) * s.jAmp;
    ...
    for (const s of stars) {
      ...
      const op  = Math.min(1, s.baseOpacity + p * 0.6) * tw;
      ...
      if (s.binaryShow > 0.06) {
        ...
        if (s.binaryShow > 0.15) {
          ...
          gg.addColorStop(0, `rgba(${tr2},${tg2},${tb2},${Math.max(s.binaryShow, p) * 0.2 * tw})`);
  ...
  window.FooterCanvas = {
    ...
    init: function () {
      canvas = document.getElementById('footer-c');
      if (!canvas) return;
      ctx = canvas.getContext('2d');

      // Canvas covers the full viewport (matches base.css: position:fixed; top:0; left:0; width:100%; height:100%)
